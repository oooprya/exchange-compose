...
# with open('par.json') as val_cur:
#     data = json.load(val_cur)
#     val_cur.close()


# for curr in data:
#   # print(curr)
#   print(f"{curr['prio']} {curr['currency']} {curr['rate']['buy']['value']} {curr['rate']['sell']['value']}")




# def add_currency():
#     x = requests.post(url_currencys, json = currency_json, headers = headers )
#     print(f"{currency_json} {x.status_code}")


# """Добавить Обменники"""
# exchangers = [{
#     "address": "вулиця Успенська, 41",
#     "address_map": "https://maps.app.goo.gl/FYfhXCcqQuLtCUdL7",
#     "working_hours": "9:00 - 19:30"
#     },
#     {
#     "address": "вулиця Мала Арнаутська, 49",
#     "address_map": "https://maps.app.goo.gl/t9e5uvLYuSzwLYK58",
#     "working_hours": "9:00 - 19:00"
#     },
#     {
#     "address": "Бізнес центр Грецька площа, 1",
#     "address_map": "https://maps.app.goo.gl/BqNwrCnnkpJnsAwBA",
#     "working_hours": "9:00 - 19:00"
#     },
#     {
#     "address": "вулиця Олексія Маркевича, 32",
#     "address_map": "https://maps.app.goo.gl/Kh8CvPGCSQMGuvR8A",
#     "working_hours": "8:00 - 18:00"
#     }]

# data_id = 1
# for exchanger in exchangers:
#     x = requests.post(f'{getenv("API")}/api/v1/exchangers/', json = exchanger, headers = headers)
...
#     x = requests.post(f'{getenv("API")}/api/v1/currency/', json = curr, headers = headers)
#     print(f'{curr} - {x.status_code}')
...
async def send_currency(new_text):
    ...
    async with bot.session:  # or `bot.context()`
        ...
        await bot.send_message(chat_id='-1002443613215',  text=new_text)
...
def edit_currency(url, new_json) -> None:
    ...
    print(f"{new_json} {x.status_code}")
    print(f'Прошло времени {time.monotonic() - now_time }')
...
def update_course(data_pars):
        ...
        for curr_api in data_api["objects"]:
            if curr['currency'] == curr_api['code']:
                if curr['buy'] != curr_api['buy'] or curr['sell'] != curr_api['sell']:
                    print(
                        f"{curr_api['id']} buy {curr['buy'] == curr_api['buy']} {curr['buy']} = {curr_api['buy']} sell {curr['sell'] == curr_api['sell']}  {curr['sell']} = {curr_api['sell']}")
                    ed_curr = {
                        "buy": f"{curr['buy']}",
                        "sell": f"{curr['sell']}"
                    }
    ...
    asyncio.run(send_currency(new_currency))


def add_course():
    """Добавляем курс на сайт"""
    data_api = get_api_data(f'{getenv("API")}/api/v1/currency')
            ...
            for curr_api in data_api["objects"]:
                ...
                if curr['currency'] == curr_api['code']:
                    ...
                    x = requests.post(f'{getenv("API")}/api/v1/currencys/',
                        ...
                        headers = headers )
                    print(x.status_code)
                    print(f"exchanger_id {exchanger_id } {curr_api['id']} {curr_api['code']} buy {curr['buy']}  sell {curr['sell']}")
            currency_id += 1
            print(curr['currency'])
        exchanger_id += 1
...
def main():
    ...
    while True:
        ...
        if now.hour >= 8 and now.hour < 20:
            ...
            print("Выполняем задачу в", now.strftime("%H:%M:%S"))
            ...
            if previous_hash is not None:
                if current_hash != previous_hash:
                    print(f"Новые курсы! Новый хеш: {current_hash}")
                    update_course(parser_exchanger())
                else:
                    print(f"Курсы не изменились. {previous_hash}")
